...
namespace detail {
  ...
  inline auto make_high_entropy_seed_array() -> std::array<uint64_t, 8>
  {
    ...
    // likely different between runs and platforms: address of this function
    const auto e6 = reinterpret_cast<uint64_t>(&make_high_entropy_seed_array);
	  return {{ e0, e1, e2, e3, e4, e5, e6, 0x2bf820b4dd7c1a8a }};
  }
...
public:
  ...
  struct param_type {
    using distribution_type = const_distribution;
    param_type() {}
    explicit param_type(T val) noexcept : val(val) {}
    T val;
  };
  ...
  explicit const_distribution(const param_type& par)
  : val_(par.val)
  {
  }

  param_type param() const { return { val_ }; }
  void param(const param_type& par) { val_ = par.val; }
  void reset() {}
  ...
  template <typename URNG>
  T operator()(URNG&, param_type const& par) const { return par.val; }
...
public:
  ...
  struct param_type
  {
    using distribution_type = uniform_signed_distribution<IntT>;
    param_type() {}
    param_type(UIntT range, IntT lo)
      : range(range), lo(lo)
    {
      digits = detail::msb_uint(UIntT(range + 1));
    }
    UIntT range;
    IntT lo;
    int digits;
  ...
  explicit uniform_signed_distribution(const param_type& par)
    : p_(par)
  {}
...
// gap_sampler()_0 = x0 + dist()
// gap_sampler()_n = gap_sampler()_n-1 + dist()
...
public:
  ...
  explicit gap_sampler(Dist&& dist, result_type x0 = 0) : dist_(std::move(dist)), x_(x0) {}

  gap_sampler& reset(result_type x0) { x_ = x0; return *this; }

  template <typename URENG>
  result_type operator()(URENG& reng) { return x_ += dist_(reng); }
...
// Use-case: sample next index in sequence if each index
// has the probability P to get picked and (1-P) to get rejected
template <typename T = int>
inline auto make_geo_gap_sampler(double p, T x0)
{
  return gap_sampler(std::geometric_distribution<T>(p), x0);
}

// Use-case: sample next time-step in Gillespie algorithm
template <typename T = double>
inline auto make_exp_gap_sampler(T lambda, T x0)
{
  return gap_sampler(std::exponential_distribution<T>(lambda), x0);
}
