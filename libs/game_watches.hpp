...
namespace game_watches {
  ...
  public:
    ...
    void tick(Duration t, Fun&& fun) const {
      ...
      // wait for sync
      while (!break_all_.load(std::memory_order_acquire)) {
        auto d = sleep_duration();
        if (d < min_sleep()) break;
        std::this_thread::sleep_for(d);
        fun();
    ...
    void break_all() const noexcept {
      break_all_.store(true, std::memory_order_release);
    }
  ...
  private:
    ...
    mutable std::atomic<bool> break_all_ = false;
  };
