...
namespace shader {
  ...
  const char* birdVertex = R"glsl(
    ...
    layout (location = 2) in vec4 pos4;
    layout (location = 3) in vec4 vel4;
    layout (location = 4) in vec4 side4;
    layout (location = 5) in vec2 tex_alpha;
    ...
    {
      vTex = TexCoord;
      vSpectrum = tex_alpha.x;
      vAlpha = tex_alpha.y;
      vec2 pos = pos4.xy;      
      vec2 dir = normalize(vel4.xy);
...
//      EmitTriangle(vec4(s, s, 0, 0));
  ...
  const char* texVertex = R"glsl(
    ...
    layout (location = 0) in vec4 Vertex;
    layout (location = 1) uniform vec4 eye = vec4(0);
  ...
  const char* ribbonVertex = R"glsl(
    ...
    void main()
    {
      int i = 4 * (first + gl_InstanceID + N * gl_VertexID);
      vec2 pos = data[i].xy;
      vec2 tex_alpha = data[i + 3].xy;
      vSpectrum = tex_alpha.x;
      vAlpha = fade * (S - gl_VertexID) * tex_alpha.y;
      vVertex = MVP * vec4(pos, 0.0, 1.0);
    }
