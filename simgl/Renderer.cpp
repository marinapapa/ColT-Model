...
namespace {
    ...
    trail(name, tconfig, J)
  {
    ...
    glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(instance_proxy), (void*)0);                            // pos
    glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(instance_proxy), (void*)sizeof(glm::vec4));            // vel
    glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(instance_proxy), (void*)(2ull * sizeof(glm::vec4)));   // side
    glVertexAttribPointer(5, 2, GL_FLOAT, GL_FALSE, sizeof(instance_proxy), (void*)(3ull * sizeof(glm::vec4)));   // tex_alpha
...
nearest_instance_record RendererImpl::nearest_instance(int mouseX, int mouseY)
{
  ...
  if (hit_test(mouseX, mouseY)) {
    auto sim_pos = glm::vec3(area_coor(mouseX, mouseY), 0.f);
    ...
    for (size_t s = 0; s < model::n_species; ++s) {
      ...
      if (pi && (pi->alpha != 0.f)) {
        for (size_t i = 0; i < gl_species_[s].size; ++i, ++pi) {
          const auto dd = glm::distance2(sim_pos, glm::vec3(pi->pos));
          if (dd < mindd) {
            nnr = { pi, s, i, dd };
            mindd = dd;
          }
...
namespace {
  ...
  void flush_species(RendererImpl* self, const model::Simulation& sim, gl_species_array& gls)
  {
    ...
    gls[I].size = static_cast<GLsizei>(sim.visit_all<std::integral_constant<size_t, I>>([psim = &sim, p = p_inst, cm = gls[I].color_map](const auto& ind, size_t idx) mutable {
...
void RendererImpl::flush_camera()
{
  if (follow_.idx >= 0) {
    ...
    const auto shift = glm::vec3(camera_->eye()) - eye;
...
void RendererImpl::cs_render()
{
  ...
  const auto eye = glm::vec3(camera_->eye());
  glUniform4f(1, eye.x, eye.y, 0.f, 0.f);
